...
APP_NAME="Summon"
BUNDLE_ID="com.lswingrover.summon"
VERSION="1.0.0"
BUILD="1"
INSTALL_DIR="/Applications"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo "▶ Summon build_app.sh v${VERSION}"
...
# ── Step 3: Icon ───────────────────────────────────────────────────────────────
echo "[3/7] Icon…"
ICNS_SRC="${SCRIPT_DIR}/Resources/summon.icns"
if [ -f "${ICNS_SRC}" ]; then
    ...
    echo "      ⚠ no icns found — add Resources/summon.icns for a custom icon"
fi
...
echo ""
echo "✅  Summon v${VERSION} installed → ${INSTALL_DIR}/${APP_NAME}.app"
echo "    Launch: open '${INSTALL_DIR}/${APP_NAME}.app'"
echo ""
echo "    NOTE: On first launch, grant Accessibility permission:"
echo "    System Settings > Privacy & Security > Accessibility > enable Summon"
